...
import type { DefenderStats } from "@/lib/defender-store";
import { RecentSweepers } from "@/components/RecentSweepers";

/**
 * Synthesis — the distributed bid-wall mission. Folded into /earn
 * (#synthesis) from the former standalone /synthesis page (itself
 * the renamed /hold-the-line). Holders place an OpenSea bid above
 * floor × 1.4 to hold the floor and earn hex.
 *
 * The async data fetch (getStats) + fire-and-forget visit tick stay
 * in the parent page; stats are passed in as a prop. Code-internal
 * defender-store naming (totalDefenders, topDefenders, DEFENDER badge)
 * intentionally left alone. Blue-doctrine "WE HEAR. WE SYNC." voice —
 * NOT combat framing.
 */
export function SynthesisSection({ stats }: { stats: DefenderStats }) {
  return (
    <>
      {/* HERO */}
