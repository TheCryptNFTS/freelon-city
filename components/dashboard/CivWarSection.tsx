/**
 * CivWarSection — weekly civilization leaderboard.
 * Folded from app/civ-wars into the /dashboard hub. Server component;
 * reuses getCivStandings() from lib/civ-wars and the MyCivStandings widget.
 */
...
export async function CivWarSection() {
  ...
  return (
    <section id="civ-war" className="civ-wars-page" style={{ scrollMarginTop: 96 }}>
      {/* Hero */}
      <section
        style={{
      ...
      >
        ...
        <h2 style={{ fontFamily: "var(--display)", fontSize: "clamp(36px, 5vw, 56px)", lineHeight: 0.95, margin: "10px 0 8px", letterSpacing: "-0.015em" }}>
          {top && top.totalHex > 0 ? (
      ...
      {/* HOW SCORING WORKS — rules before results. */}
          ...
          {top3.map((c, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 style={{ fontFamily: "var(--display)", fontSize: isFirst ? 36 : 22, margin: 0, color: "var(--ink)", lineHeight: 1 }}>{c.name}</h3>
      ...
      {/* Full standings */}
      <section>
        <h3 className="kicker" style={{ marginBottom: 12 }}>⬡ ALL CIVILIZATIONS</h3>
