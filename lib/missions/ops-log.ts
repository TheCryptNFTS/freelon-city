...
type Mem = { runs: Map<string, number>; cost: Map<string, number>; img: Map<string, number>; errors: OpsError[] };
const mem: Mem = ((globalThis as { __freelonOpsMem?: Mem }).__freelonOpsMem ??= {
  runs: new Map(), cost: new Map(), img: new Map(), errors: [],
});
...
export async function opsSnapshot(): Promise<OpsSnapshot> {
  let runs = 0, images = 0, microUsd = 0;
  let errors: OpsError[] = [];
  if (hasUpstash) {
    try {
      const [r, c, i, e] = (await Promise.all([
        upstash(["GET", K_RUNS()]),
        upstash(["GET", K_COST()]),
        upstash(["GET", K_IMG()]),
        upstash(["LRANGE", K_ERR, "0", "24"]),
      ])) as [string | null, string | null, string | null, string[] | null];
      runs = Number(r ?? 0); microUsd = Number(c ?? 0); images = Number(i ?? 0);
  ...
  return { day: utcDay(), runs, images, estCostUsd: Math.round(microUsd / 10000) / 100, recentErrors: errors };
}
