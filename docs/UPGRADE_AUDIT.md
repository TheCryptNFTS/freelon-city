...
### DEFERRED — larger builds
**SHIPPED after the initial r2 (Billy "/goal achieve all"):** idx115 (demo→shareable image quote card — new /api/og/quote + /share/quote landing + tweetDemoReply wired; **live render verified on-brand**), idx116 (demo MEMORY preview panel — visualizes what an owned citizen keeps with an honest empty state; demo stays stateless by design so the paid-feature differentiator is preserved), idx73 (DailyHub server-claim link-out row), idx14 (OG brand font — FreelonCard kicker **and** the citizen-share card name).
**Still deferred (legit reasons, not blockers):** idx26 (weekly play-event founder metric — low user value, touches the live auto-posting cron), idx40 (lazy-load /play overlays — the finding says measure LCP before/after first), idx8 (trim the demo-wall exit stack — tension with the shipped /proof surfacing #18), idx33 (--neon-cyan rename — cosmetic churn across ~6 files), and the UniverseCard kicker font (the rarely-served ?v=universe variant).
